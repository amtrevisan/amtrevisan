...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useFrame((state, delta) => {
    ...
    // Don't set clear color - keep it transparent
  });
  ...
  const geo = (nodes[geometryKey] as THREE.Mesh)?.geometry;
  if (!geo) {
    console.error(`Geometry "${geometryKey}" not found in ${glb}`);
    return null;
  }
...
function Bar({ modeProps = {}, ...p }: { modeProps?: ModeProps } & MeshProps) {
  ...
  return (
    <ModeWrapper
      ...
      lockToBottom
      followPointer={false}
      modeProps={{ ...defaultMat, scale: 0.5, ...modeProps }}
      {...p}
    />
  );
}
...
function NavItems({ items }: { items: NavItem[] }) {
  ...
  const DEVICE = {
    mobile: { max: 639, spacing: 0.8, fontSize: 0.1 },
    tablet: { max: 1023, spacing: 1.2, fontSize: 0.12 },
    desktop: { max: Infinity, spacing: 1.5, fontSize: 0.14 }
  };
  const getDevice = () => {
  ...
  useFrame(() => {
    ...
    const v = viewport.getCurrentViewport(camera, [0, 0, 15]);
    group.current.position.set(0, -v.height / 2 + 0.25, 15.5);
