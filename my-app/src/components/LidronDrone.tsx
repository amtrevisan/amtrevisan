...
// Preload the Lidron drone model
useGLTF.preload('/models/LidronDrone.glb')

function LidronDroneModel({ scrollProgress }: { scrollProgress: number }) {
  const groupRef = useRef<THREE.Group>(null)
  const gltf = useGLTF('/models/LidronDrone.glb')
  ...
  useFrame(() => {
    if (groupRef.current) {
      ...
      // Add propeller rotation animation if present
...
export default function LidronDrone() {
    ...
    <div ref={containerRef} className="w-full h-full flex items-center justify-center">
      ...
      >
        ...
        <Suspense fallback={<Loader />}>
          <LidronDroneModel scrollProgress={scrollProgress} />
        </Suspense>
      </Canvas>
    </div>
  )
